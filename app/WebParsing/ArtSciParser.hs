...
parseBuildings = do
    ...
    -- buildingInfo <- getBuildingList bodyTags
    buildingInfo <- getBuildingsFromCSV buildingCSV
...
-- getBuildingList :: [Tag T.Text] -> IO [Building]
...
--         buildings = liftM (filter (\b -> not $ T.null (buildingAddress b))) $ mapM extractBuildings listing'
--     in buildings
--     where
--         extractBuildings :: [Tag T.Text] -> IO Building
--         extractBuildings listTags = do
...
getBuildingsFromCSV buildsCSVFile = do
    ...
    case buildsCSVData of
        ...
        Right buildsData -> do
            -- let buildsList = drop 0 $ map (\b -> map (\bdata -> T.pack bdata) b ) buildsData
